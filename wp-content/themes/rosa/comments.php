		...
		<div class="comments-area-title">
			<h3 class="comments-title">
				...
				if ( have_comments() ) :
					...
					_e( '<span class="comment-number total">+</span> There are no comments', 'rosa_txtd' );
				endif;
				?>
			</h3>
			<?php echo '<a class="comments_add-comment" href="#reply-title">' . __( 'Add yours', 'rosa_txtd' ) . '</a>'; ?>
		</div>
		<?php
		...
		if ( have_comments() ) :
			if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through ?>
				<nav role="navigation" id="comment-nav-above" class="site-navigation comment-navigation">
					<h3 class="assistive-text"><?php _e( 'Comment navigation', 'rosa_txtd' ); ?></h3>

					<div class="nav-previous"><?php previous_comments_link( __( '&larr; Older Comments', 'rosa_txtd' ) ); ?></div>
					<div class="nav-next"><?php next_comments_link( __( 'Newer Comments &rarr;', 'rosa_txtd' ) ); ?></div>
				</nav><!-- #comment-nav-before .site-navigation .comment-navigation -->
			<?php endif; // check for comment navigation ?>

			<ol class="commentlist">
				<?php
				/*  for PRO users! -  Loop through and list the comments. Tell wp_list_comments()
...
if ( ! comments_open() && post_type_supports( get_post_type(), 'comments' ) && ! is_page() ) :
	?>
	<p class="nocomments"><?php _e( 'Comments are closed.', 'rosa_txtd' ); ?></p>
<?php endif;
...
if ( is_user_logged_in() ) {
	...
	$comments_args = array(
		...
		'title_reply'          => __( '<span class="comment-number total">+</span> Leave a Comment', 'rosa_txtd' ),
		...
		'fields'               => apply_filters( 'comment_form_default_fields', array(
			...
			'email'  => '<p class="comment-form-email"><label for="email" class="show-on-ie8">' . __( 'Email', 'rosa_txtd' ) . '</label><input id="email" name="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" type="text" placeholder="' . __( 'your@email.com', 'rosa_txtd' ) . '..." ' . $aria_req . ' /></p>'
		) ),
