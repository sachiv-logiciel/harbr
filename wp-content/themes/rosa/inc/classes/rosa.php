...
class rosa {
	...
	/*  for PRO users! - * Limit words for a string */
	...
	static function the_archive_title() {
		...
		if ( is_home() ) { ?>
			<h1 class="hN  archive__title">
                <?php  if( isset($object->post_title)) echo $object->post_title; else _e( 'News', 'rosa_txtd' ); ?></h1>
            <hr class="separator"/>
		<?php
		} elseif ( is_search() ) {
			?>
			<div class="heading headin--main">
				<span class="archive__side-title beta"><?php _e( 'Search Results for: ', 'rosa_txtd' ) ?></span>
		...
		<?php
			...
			<div class="heading headin--main">
				...
				<span class="archive__side-title beta"><?php _e( 'Tag', 'rosa_txtd' ) ?></span>
			</div>
			<hr class="separator"/>
		<?php } elseif ( ! empty( $object ) && isset( $object->term_id ) ) { ?>
			<div class="heading headin--main">
				<h1 class="archive__title"><?php echo $object->name; ?></h1>
				<span class="archive__side-title beta"><?php _e( 'Category', 'rosa_txtd' ) ?></span>
			</div>
			<hr class="separator"/>
		<?php } elseif ( is_day() ) { ?>
			<div class="heading headin--main">
				<span class="archive__side-title beta"><?php _e( 'Daily Archives: ', 'rosa_txtd' ) ?></span>
		...
		<?php } elseif ( is_month() ) { ?>
			<div class="heading headin--main">
				<span class="archive__side-title beta"><?php _e( 'Monthly Archives: ', 'rosa_txtd' ) ?></span>

				<h1 class="archive__title"><?php echo get_the_date( _x( 'F Y', 'monthly archives date format', 'rosa_txtd' ) ); ?></h1>
			</div>
			<hr class="separator"/>
		<?php } elseif ( is_year() ) { ?>
			<div class="heading headin--main">
				<span class="archive__side-title beta"><?php _e( 'Yearly Archives: ', 'rosa_txtd' ) ?></span>

				<h1 class="archive__title"><?php echo get_the_date( _x( 'Y', 'yearly archives date format', 'rosa_txtd' ) ); ?></h1>
			</div>
			<hr class="separator"/>
		<?php } else { ?>
			<div class="heading headin--main">
				<span class="archive__side-title beta"><?php _e( 'Archives', 'rosa_txtd' ) ?></span>
			</div>
			<hr class="separator"/>
		<?php
		}
				...
				<div class="flexbox__item">
					<?php if ( $style == 'dropdown' ) : ?>
						<div class="pix-dropdown  up  border-menu">
							<a class="dropdown__trigger" href="#"><?php echo __( 'Filter', 'rosa_txtd' ) ?>
							...
							<ul class="dropdown__menu  nav  nav--banner  archive-categories-list  border-menu">
								<?php foreach ( $filter_categories as $key => $category ) { ?>
									<li class="category-item archive__filter-item">
										<a href="#" title="<?php echo sprintf( __( "View all in %s", 'rosa_txtd' ), $category->name ); ?>" data-filter="<?php echo '.cat-' . $category->slug; ?>"><?php echo $category->name; ?></a>
									</li>
								<?php } ?>
								<li class="category-item archive__filter-item" data-filter="all">
									<a href="#" title="<?php echo __( 'View all', 'rosa_txtd' ) ?>" data-filter="*"><?php _e( 'View All', 'rosa_txtd' ); ?></a>
								</li>
							</ul>
						</div>
					<?php else: ?>
	...
	static function the_author_posts_link() {
		...
		$link = sprintf( '<a href="%1$s" title="%2$s">%3$s</a>', esc_url( get_author_posts_url( $authordata->ID, $authordata->user_nicename ) ), esc_attr( sprintf( __( 'Posts by %s', 'rosa_txtd' ), get_the_author() ) ), get_the_author() );
