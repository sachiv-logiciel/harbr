...
function wpgrade_custom_post_gallery( $output, $attr ) {
	...
	if ( "true" === $mkslideshow ) {
		...
		<style type='text/css'>
			...
			/*  for PRO users! -  see gallery_shortcode() in wp-includes/media.php */
		</style>\n\t\t";
		}
		...
		function add_video_url_field_to_attachments( $form_fields, $post ) {
			if ( ! isset( $form_fields["video_url"] ) ) {
				$form_fields["video_url"] = array(
					"label" => __( "Video URL", 'rosa_txtd' ),
					"input" => "text", // this is default if "input" is omitted
					"value" => esc_url( get_post_meta( $post->ID, "_video_url", true ) ),
					"helps" => __( "<p class='desc'>Attach a video to this image <span class='small'>(YouTube or Vimeo)</span>.</p>", 'rosa_txtd' ),
				);
			}
			...
			if ( ! isset( $form_fields["video_autoplay"] ) ) {
				...
				$form_fields["video_autoplay"] = array(
					"label" => __( "Video  Autoplay", 'rosa_txtd' ),
					"input" => "html",
					"html"  => '<input' . $checked . ' type="checkbox" name="attachments[' . $post->ID . '][video_autoplay]" id="attachments[' . $post->ID . '][video_autoplay]" /><label for="attachments[' . $post->ID . '][video_autoplay]">' . __( 'Enable Video Autoplay?', 'rosa_txtd' ) . '</label>'
...
//					"label" => __( "External URL", 'rosa_txtd' ),
//					"input" => "text",
//					"value" => esc_url( get_post_meta( $post->ID, "_external_url", true ) ),
//					"helps" => __( "<p class='desc'>Set this image to link to an external website.</p>", 'rosa_txtd' ),
//				);
//			}
...
function wpgrade_custom_gallery_settings() {
	...
	<script type="text/html" id="tmpl-mkslideshow">
		<label class="setting">
			<span><?php _e( 'Make this gallery a slideshow', 'rosa_txtd' ) ?></span>
