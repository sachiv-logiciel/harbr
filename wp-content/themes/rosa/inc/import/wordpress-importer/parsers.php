...
class WXR_Parser {
	function parse( $file ) {
		...
		if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
			...
			echo '</pre>';
			echo '<p><strong>' . __( 'There was an error when reading this WXR file', 'rosa_txtd' ) . '</strong><br />';
			echo __( 'Details are shown above. The importer will now try again with a different parser...', 'rosa_txtd' ) . '</p>';
		}
...
class WXR_Parser_SimpleXML {
	function parse( $file ) {
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new WP_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'rosa_txtd' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version ) {
			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'rosa_txtd' ) );
		}
