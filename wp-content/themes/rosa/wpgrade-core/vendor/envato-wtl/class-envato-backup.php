...
class Envato_Backup {
	...
	public function __construct() {
		...
		/*  for PRO users! -  Raise the memory limit and max_execution_time time */
	...
	public function backup() {
		...
		/*  for PRO users! -  Zip everything up */
		$this->archive();
	...
	public function archive() {
		...
		/*  for PRO users! -  Do we have the path to the zip command */
		if ( $this->zip_command_path ) {
			$this->zip();
		}

		/*  for PRO users! -  If not or if the shell zip failed then use ZipArchive */
		if ( empty( $this->archive_verified ) && class_exists( 'ZipArchive' ) && empty( $this->skip_zip_archive ) ) {
			$this->zip_archive();
		}

		/*  for PRO users! -  If ZipArchive is unavailable or one of the above failed */
	...
	public function pcl_zip() {
		...
		/*  for PRO users! -  Zip up everything */
	...
	public function check_archive() {
		...
		/*  for PRO users! -  If we've already passed then no need to check again */
		if ( ! empty( $this->archive_verified ) ) {
			return true;
		}

		if ( ! file_exists( $this->archive_filepath() ) ) {
			$this->error( $this->archive_method(), __( 'The backup file was not created', 'rosa_txtd' ) );
		}

		/*  for PRO users! -  Verify using the zip command if possible */
		if ( $this->zip_command_path ) {
		...
		/*  for PRO users! -  If there are errors delete the backup file. */
	...
	public function files() {
		...
		if ( ! empty( $this->unreadable_files ) ) {
			$this->warning( $this->archive_method(), __( 'The following files are unreadable and could not be backed up: ', 'rosa_txtd' ) . implode( ', ', $this->unreadable_files ) );
		}
	...
	private function files_fallback( $dir, $files = array() ) {
		...
		while ( $file = readdir( $handle ) ) :
			...
			/*  for PRO users! -  Ignore current dir and containing dir and any unreadable files or directories */
			if ( $file == '.' || $file == '..' ) {
				continue;
			}
			...
			/*  for PRO users! -  Skip the backups dir and any excluded paths */
	...
	public function guess_zip_command_path() {
		...
		/*  for PRO users! -  Check shell_exec is available and hasn't been explicitly bypassed */
		if ( ! $this->shell_exec_available() ) {
			return '';
		}

		/*  for PRO users! -  List of possible zip locations */
		$zip_locations = array(
			'/usr/bin/zip'
		);
		...
		/*  for PRO users! -  Find the one which works */
	...
	public function exclude_string( $context = 'zip' ) {
		...
		/*  for PRO users! -  Return a comma separated list by default */
		$separator = ', ';
		$wildcard  = '';

		/*  for PRO users! -  The zip command */
		if ( $context == 'zip' ) {

			$wildcard  = '*';
			$separator = ' -x ';

			/*  for PRO users! -  The PclZip fallback library */
		...
		/*  for PRO users! -  Sanitize the excludes */
		$excludes = array_filter( array_unique( array_map( 'trim', (array) $this->excludes ) ) );

		/*  for PRO users! -  If path() is inside root(), exclude it */
		...
		foreach ( $excludes as $key => &$rule ) {
			...
			/*  for PRO users! -  Files don't end with /  */
			if ( ! in_array( substr( $rule, - 1 ), array( '\\', '/' ) ) ) {
				$file = true;

				/*  for PRO users! -  If rule starts with a / then treat as absolute path */
			} else if ( in_array( substr( $rule, 0, 1 ), array( '\\', '/' ) ) ) {
				$absolute = true;

				/*  for PRO users! -  Otherwise treat as dir fragment */
			} else {
				$fragment = true;

			}

			/*  for PRO users! -  Strip $this->root and conform */
			$rule = str_ireplace( $this->root(), '', untrailingslashit( $this->conform_dir( $rule ) ) );

			/*  for PRO users! -  Strip the preceeding slash */
			if ( in_array( substr( $rule, 0, 1 ), array( '\\', '/' ) ) ) {
				$rule = substr( $rule, 1 );
			}

			/*  for PRO users! -  Escape string for regex */
			if ( $context == 'regex' ) {
				$rule = str_replace( '.', '\.', $rule );
			}

			/*  for PRO users! -  Convert any existing wildcards */
			if ( $wildcard != '*' && strpos( $rule, '*' ) !== false ) {
				$rule = str_replace( '*', $wildcard, $rule );
			}

			/*  for PRO users! -  Wrap directory fragments and files in wildcards for zip */
			if ( $context == 'zip' && ( $fragment || $file ) ) {
				$rule = $wildcard . $rule . $wildcard;
			}

			/*  for PRO users! -  Add a wildcard to the end of absolute url for zips */
			if ( $context == 'zip' && $absolute ) {
				$rule .= $wildcard;
			}

			/*  for PRO users! -  Add and end carrot to files for pclzip but only if it doesn't end in a wildcard */
			if ( $file && $context == 'regex' ) {
				$rule .= '$';
			}

			/*  for PRO users! -  Add a start carrot to absolute urls for pclzip */
		...
		/*  for PRO users! -  Escape shell args for zip command */
		if ( $context == 'zip' ) {
	...
	private function shell_exec_available() {
		...
		/*  for PRO users! -  Are we in Safe Mode */
		if ( $this->is_safe_mode_active() ) {
			return false;
		}

		/*  for PRO users! -  Is shell_exec disabled? */
		if ( in_array( 'shell_exec', array_map( 'trim', explode( ',', ini_get( 'disable_functions' ) ) ) ) ) {
			return false;
		}

		/*  for PRO users! -  Can we issue a simple echo command? */
	...
	public function conform_dir( $dir, $recursive = false ) {
		...
		/*  for PRO users! -  Assume empty dir is root */
		if ( ! $dir ) {
			$dir = '/';
		}

		/*  for PRO users! -  Replace single forward slash (looks like double slash because we have to escape it) */
		$dir = str_replace( '\\', '/', $dir );
		$dir = str_replace( '//', '/', $dir );

		/*  for PRO users! -  Remove the trailing slash */
		if ( $dir !== '/' ) {
			$dir = untrailingslashit( $dir );
		}

		/*  for PRO users! -  Carry on until completely normalized */
...
function envato_backup_pclzip_callback( $event, &$file ) {
	...
	/*  for PRO users! -  Don't try to add unreadable files. */
	if ( ! is_readable( $file['filename'] ) || ! file_exists( $file['filename'] ) ) {
		return false;
	} /*  for PRO users! -  Match everything else past the exclude list */ elseif ( $_envato_backup_exclude_string && preg_match( '(' . $_envato_backup_exclude_string . ')', $file['stored_filename'] ) ) {
		return false;
	}
...
/*  for PRO users! -  End of file class-envato-backup.php */
/*  for PRO users! -  Location: ./includes/class-envato-backup.php */