...
class Envato_Protected_API {
	...
	public function __construct( $user_name = '', $api_key = '' ) {
		...
		if ( $user_name == '' ) {
			$this->set_error( 'user_name', __( 'Please enter your Envato Marketplace Username.', 'rosa_txtd' ) );
		}

		if ( $api_key == '' ) {
			$this->set_error( 'api_key', __( 'Please enter your Envato Marketplace API Key.', 'rosa_txtd' ) );
		}
	...
	public function private_user_data( $set = '', $user_name = '', $set_data = '', $allow_cache = false, $timeout = 300 ) {
		...
		if ( $set == '' ) {
			$this->set_error( 'set', __( 'The API "set" is a required parameter.', 'rosa_txtd' ) );
		}
		...
		/*  for PRO users! -  set transient ID for later */
	...
	public function wp_download( $item_id ) {
		...
		if ( ! isset( $item_id ) ) {
			$this->set_error( 'item_id', __( 'The Envato Marketplace "item ID" is a required parameter.', 'rosa_txtd' ) );
		}
	...
	public function set_cache( $transient = '', $url = '', $timeout = 300 ) {
		...
		/*  for PRO users! -  keep the code below cleaner */
		$transient         = $this->validate_transient( $transient );
		$transient_timeout = '_transient_timeout_' . $transient;

		/*  for PRO users! -  set original cache before we destroy it */
		$old_cache = get_option( $transient_timeout ) < time() ? get_option( $transient ) : '';

		/*  for PRO users! -  look for a cached result and return if exists */
		if ( false !== $results = get_transient( $transient ) ) {
			return $results;
		}

		/*  for PRO users! -  create the cache and allow filtering before it's saved */
...
/*  for PRO users! -  End of file class-envato-api.php */
/*  for PRO users! -  Location: ./includes/class-envato-api.php */